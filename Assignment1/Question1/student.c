...
void merge(int* nums1, int nums1Size, int m, int* nums2, int nums2Size, int n) {
    (void)nums1Size;
    (void)nums2Size;
    int i=m-1; int j=n-1; int k=m+n-1; //three pointers, two at nums1 m and nums2 n, and one index at the end of nums1
        while (i>=0 && j>=0){
            if (nums1[i]>=nums2[j]){ //comparing elements of both arrays
            nums1[k]=nums1[i];
            i--;
            k--;
            } else {
